...
struct SoundboardPanelView: View {
        ...
        PanelShell(
            ...
            trailingToolbar: {
                ToolbarIconButton(
                    systemName: "plus",
                    action: {
                        openWindow(value: ImportAssetWindowPayload(initialKind: .sfx))
                    },
                ...
                if isSearching {
                    ...
                    ForEach(searchResults, id: \.id) { item in
                        PanelRow(
                            title: item.title,
                            icon: "play.fill",
                            isBookmarked: bookmarks.isSFXBookmarked(item.title),
                            toggleBookmark: { bookmarks.toggleSFX(item.title) },
                            isPlaying: playback.isSFXPlaying(item.title),
                            togglePlay: {
                                let wasPlaying = playback.isSFXPlaying(item.title)
                                playback.toggleSFX(item.title)
                                if wasPlaying {
                                    backend.audioManager.stopSFX(title: item.title)
                                } else {
                                    backend.audioManager.playSFX(title: item.title)
                                }
                            },
                        ...
                        .contextMenu {
                            Button {
                                bookmarks.toggleSFX(item.title)
                            } label: {
                                Label(
                                    bookmarks.isSFXBookmarked(item.title) ? "Remove bookmark" : "Bookmark this sound",
                                    systemImage: bookmarks.isSFXBookmarked(item.title) ? "bookmark.slash" : "bookmark"
                                )
                            }
                            Button {
                                editTarget = EditTarget(itemId: item.id, title: item.title, kind: .sfx)
                            } label: {
    ...
    private func loadFacets() async {
        ...
        do {
            ...
            facets = try await backend.facetRepository.facetCounts(kind: .sfx, filters: Filters())
        } catch {
    ...
    private func tagOptions(for category: SortCategory) -> [TagOption] {
        ...
        case .all:
            return []
        case .theme:
            source = facets.sfxThemes
        case .location:
            source = facets.locations
        case .creatures:
            source = facets.creatureTypes
        }
        return source
            .filter { $0.0.name != "All" }
    ...
    private func filter(for category: SortCategory, tagId: Int64) -> Filters {
        ...
        case .all:
            return filters
        case .theme:
            filters.sfxThemeIDs = [tagId]
        case .location:
            filters.locationIDs = [tagId]
        case .creatures:
            filters.creatureTypeIDs = [tagId]
        }
        return filters
    }
    ...
    private func loadItems(for option: TagOption) async {
        ...
        do {
            taggedItems = try await backend.libraryRepository.fetchItems(kind: .sfx, filters: filters, sort: .titleAsc)
        } catch {
            print("Failed to load sfx: \(error)")
            taggedItems = []
        }
    ...
    private func loadAllItems() async {
        ...
        isLoadingAll = true
        do {
            allItems = try await backend.libraryRepository.fetchItems(kind: .sfx, filters: Filters(), sort: .titleAsc)
        } catch {
            allItems = []
        }
        isLoadingAll = false
    ...
    private var recentSet: Set<String> {
        Set(playback.recentSFX.filter { !bookmarks.sfxBookmarks.contains($0) })
    }


    private func recencyText(for title: String) -> String? {
        guard !playback.isSFXPlaying(title) else { return nil }
        guard let lastPlayed = playback.lastPlayedSFX(title) else { return nil }
    ...
    private func performSearch(term: String) async {
        do {
            let results = try await backend.libraryRepository.searchSFXAssets(query: term)
            await MainActor.run {
    ...
    private func deleteItem(_ target: DeleteTarget) async {
        ...
        do {
            ...
            if bookmarks.isSFXBookmarked(target.title) {
                bookmarks.toggleSFX(target.title)
            }
            playback.removeRecentSFX(target.title)
